...
// ---------------------------------------------------------------------------
function resolveZoweSSH(): { host: string; port: number; user?: string; keyFile?: string } {
  try {
...
mkdir(CONTROL_DIR, { recursive: true }).catch(() => { /* ignore */ });
 ...
 */
function runSSH(remoteCmd: string, stdin?: string): Promise<string> {
...
function buildJclxArgs(opts: {
  output?: "json" | "text" | "rexx";
  dsnCheck?: "ON" | "OFF" | "DISP";
  ...
  if (opts.dsnCheck)  args.push("-c", opts.dsnCheck);
  if (opts.logLevel)  args.push("-l", opts.logLevel);
  if (opts.rulesFile) args.push("-r", opts.rulesFile);
  if (opts.jobClass)  args.push("-j", opts.jobClass);
  if (opts.aliasDsn)  args.push("-a", opts.aliasDsn);
  return args;
}
 ...
 */
async function runJclx(ussPath: string, opts: Parameters<typeof buildJclxArgs>[0]): Promise<string> {
 ...
 */
async function runJclxOnContent(
  jclContent: string,
  filename: string,
  opts: Parameters<typeof buildJclxArgs>[0],
): Promise<string> {
  const tmpUssPath = `/tmp/jclx-${Date.now()}-${basename(filename)}`;
  const quotedArgs = buildJclxArgs(opts).map((a) => `'${a}'`).join(" ");
  const remoteCmd  =
    `cat > '${tmpUssPath}' && { JCLX_NLS='${JCLX_NLS}' '${JCLX_BIN}' ${quotedArgs} '${tmpUssPath}'; ` +
 ...
 */
async function uploadAndAnalyse(localPath: string, opts: Parameters<typeof buildJclxArgs>[0]): Promise<string> {
...
// ---------------------------------------------------------------------------
server.registerTool(
  "analyse-jcl-local-file",
  {
    description:
      "Run IBM JCL Expert (jclx) against a local JCL file on the workstation. " +
    ...
    inputSchema: z.object({
      localPath: z.string().describe(
        "Absolute or workspace-relative path to the local JCL file, e.g. src/jcl/RUNNBR.jcl"
      ),
...
// ---------------------------------------------------------------------------
server.registerTool(
  "analyse-jcl-uss-file",
  {
    description:
      "Run IBM JCL Expert (jclx) against a JCL file that already exists on USS. " +
      "Returns jclx findings in the requested format (default: text).",
    inputSchema: z.object({
      ussPath: z.string().describe(
        "Absolute USS path to the JCL file, e.g. /u/drice/jcl/myjob.jcl"
      ),
      output: z.enum(["json", "text", "rexx"]).optional().default("text")
        .describe("Output format for jclx results (default: text)"),
      dsnCheck: z.enum(["ON", "OFF", "DISP"]).optional()
...
server.registerTool(
  ...
  {
    description:
      "Run IBM JCL Expert (jclx) against inline JCL text. " +
...
async function main() {
  ...
  console.error(
    `jcl-expert MCP server running on stdio (ssh → ${SSH_HOST}:${SSH_PORT}, ControlMaster enabled)`
  );
}
